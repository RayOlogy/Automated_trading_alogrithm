function  [mu, Q, adj_Rsquare] = FF(returns, factRet, lambda, K)
    
    % Use this function to perform a the Fama-French 3-factor model.
    
    factRet = factRet(:,1:3);
    
    % Number of observations and factors
    [T, p] = size(factRet); 
    
    % Data matrix
    X = [ones(T,1) factRet];
    
    % Regression coefficients
    B = (X' * X) \ X' * returns;
    
    % Separate B into alpha and betas
    a = B(1,:)';     
    V = B(2:end,:); 
    
    % Residual variance
    ep       = returns - X * B;
    sigma_ep = 1/(T - p - 1) .* sum(ep .^2, 1);
    D        = diag(sigma_ep);
    
    % Factor expected returns and covariance matrix
    f_bar = mean(factRet,1)';
    F     = cov(factRet);
    
    % Calculate the asset expected returns and covariance matrix
    mu = a + V' * f_bar;
    Q  = V' * F * V + D;
    
    % Sometimes quadprog shows a warning if the covariance matrix is not
    % perfectly symmetric.
    Q = (Q + Q')/2;
    
     % Adjusted R-square
    SSE = sum(ep.^2, 1); % Sum of squared errors
    SST = sum((returns - mean(returns, 1)).^2, 1); % Total sum of squares
    adj_Rsquare = mean(1 - (SSE/(T-p-1))./(SST/(T - 1)));
    
    %----------------------------------------------------------------------
    
end